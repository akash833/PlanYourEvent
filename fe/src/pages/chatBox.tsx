import React, { useEffect, useState, useRef } from "react";
import Layout from "./components/layout";
import { useRouter } from "next/router";
import { FiSearch, FiSend, FiArrowLeft } from "react-icons/fi";
import { io } from "socket.io-client";
import { CALL_GET_API } from "@/api";
import AppLoader from "./components/utility/loader";

let socket: any;

const chatBox = () => {
  const router = useRouter();
  const [groups, setGroups] = useState<any[]>([]);
  const [messages, setMessages] = useState<any[]>([]);
  const [loggedInUserId, setLoggedInUserId] = useState("");
  const [currentGroupId, setCurrentGroupId] = useState("");
  const [messageText, setMessageText] = useState("");
  const [events, setEvents] = useState([]);
  const [OfferPrice, setOfferPrice] = useState("");
  const [OfferHour, setOfferHour] = useState("");
  const [selectedEvent, setSelectedEvent] = useState("");
  const [provider, setProvider] = useState(false);
  const [startChatImg, setStartChatImg] = useState(true);
  const [style, setStyle] = useState(false);
  const [loader, setLoader] = useState(false);
  const [isTyping, setIsTyping] = useState(false);
  const [eventDetailId, setEventDetailId] = useState(null);
  const [eventInfo, setEventInfo] = useState<any>({});
  const [recieverImage, setRecieverImage] = useState(null);
  const chatContainerRef = useRef(null);
  const groupContainerRef = useRef(null);

  const handleInputChange = (event: any) => {
    var searchVal = event.target.value;
    if (searchVal === "") {
      ("");
      return;
    }
    const filterBySearch = groups.filter((item) => {
      if (item.name.toLowerCase().includes(searchVal.toLowerCase())) {
        return item;
      }
    });
    setGroups(filterBySearch);
  };
  const sendHourMoney = (e: any, button: any) => {
    if (button == "forHour") {
      letCheckPriceHour(e);
      const minValue = 0;
      const maxValue = 8;
      const maxLength = 8;

      let newValue = e.target.value;
      newValue = newValue.replace(/[^0-9]/g, "");
      newValue = Math.min(maxValue, Math.max(minValue, newValue)); // Apply min and max
      const maxHour = newValue.toString().slice(0, maxLength);
      setOfferHour(maxHour);
    } else if (button == "forMoney") {
      setOfferPrice(e.target.value);
      letCheckPriceHour(e);
      console.log(style, isTyping);
    }
  };

  const letCheckPriceHour = (e: any) => {
    let maxValue = 99999;
    let price = parseInt(OfferPrice);
    let hour = parseInt(OfferHour);
    if (e.target.value == 1 && price > maxValue) {
      const minValue = 0;
      const maxLength = 8;

      let newValue: any = price.toString();
      newValue = newValue.replace(/[^0-9]/g, "");
      newValue = Math.min(maxValue, Math.max(minValue, newValue)); // Apply min and max
      const maxPrice = newValue.toString().slice(0, maxLength);
      setOfferPrice(maxPrice);
    }
    const changeInNum = parseInt(e.target.value);
    if (hour && changeInNum > 99999) {
      const minValue = 0;
      const maxLength = 8;
      // let val = e.target.value;
      let newValue: any = e.target.value;
      newValue = newValue.replace(/[^0-9]/g, "");
      newValue = Math.min(maxValue, Math.max(minValue, newValue)); // Apply min and max
      const maxPrice = newValue.toString().slice(0, maxLength);
      setOfferPrice(maxPrice);
    }
  };

  const OffersButton = async (button: any, id: any, e: any) => {
    console.log(e, setLoader);
    var action = "";
    var action_performed = false;
    if (button === "accept") {
      action = "Accept";
      action_performed = true;
    } else if (button === "decline") {
      action = "Decline";
      action_performed = false;
    }

    const msg = {
      _id: id,
      groupId: currentGroupId,
      senderId: loggedInUserId,
      action_performed: action_performed,
      action: action,
    };
    socket.emit("offer updated", msg);
  };

  const getGroupLastMessageText = (group: any) => {
    if (group && group.lastMessage) {
      if (group.lastMessage.senderId == loggedInUserId) {
        return "You - " + group.lastMessage.message;
      } else {
        return group.lastMessage.message;
      }
    } else {
      return "No Conversations,  Say Hi....";
    }
  };

  const getEvents = async () => {
    const { success, data } = await CALL_GET_API(
      "events/get-event-without-pagination"
    );

    if (success) {
      setEvents(data.data);
    }
  };

  const loadGroupMessage = async (groupId: string) => {
    if (window.innerWidth <= 992) {
      setStyle(true);
    }
    setStartChatImg(false);
    const { success, data } = await CALL_GET_API(
      "group_messages/messages?groupId=" + groupId
    );
    if (success) {
      setMessages(data.messages);
    }
  };

  const setupSocket = (userId: string) => {
    console.log("Re connecting");

    setLoggedInUserId(userId);
    socket = io(
      process.env.NEXT_PUBLIC_API_PATH!
    );
    socket.on("connect", () => {
      socket.emit("userInfo", userId);
    });

    socket.on("groups", (groups: any) => {
      // console.log(groups);

      socket.emit("userInfo", userId);
      setGroups(groups);
      const urlParams = new URLSearchParams(window.location.search);
      const refId = urlParams.get("ref");
      if (refId) {
        setCurrentGroupId(refId!);
        loadGroupMessage(refId);
      }
    });

    socket.on("chat message", (packet: any) => {
      setMessages((previous) => {
        return [...previous, packet.message];
      });
    });

    socket.on("offer updated", (packet: any) => {
      setMessages((previous) => {
        return (previous || []).map((m) => {
          if (m._id == packet.message._id) {
            return packet.message;
          } else {
            return { ...m };
          }
        });
      });
    });

    socket.on("send offer", (packet: any) => {
      setMessages((previous) => {
        return [...previous, packet.message];
      });
    });

    socket.on("typingMsg", () => setIsTyping(true));
  };
  let typingTimeout: any;

  const sendReply = (e: any) => {
    const msg = {
      groupId: currentGroupId,
      senderId: loggedInUserId,
      createdAt: new Date(),
    };
    if (e.target.name == "InputMsg") {
      socket.emit("typingMsg", msg);
      clearTimeout(typingTimeout);
      typingTimeout = setTimeout(userStoppedTyping, 10000);
    }
    setMessageText(e.target.value);
  };

  function userStoppedTyping() {
    setIsTyping(false);
  }

  const isCurrentGroup = (groupId: string) => {
    return groupId == currentGroupId;
  };

  const reloadGroup = (groupId: string, image: any) => {
    const urlParams = new URLSearchParams(window.location.search);
    const refId = urlParams.get("ref");
    if (refId !== null) {
      urlParams.delete("ref");
      const newUrl = `${window.location.pathname}`;
      window.history.replaceState({}, document.title, newUrl);
    }

    if (window.innerWidth <= 992) {
      const chatRightDiv = document.getElementById("chatRightDiv");
      const chatLeftDiv = document.getElementById("chatLeftDiv");
      chatRightDiv?.classList.remove("chatRightDivNone");
      chatLeftDiv?.classList.remove("chatLeftDivBlock");
      chatRightDiv?.classList.add("chatRightDivBlock");
      chatLeftDiv?.classList.add("chatLeftDivNone");
    }
    setCurrentGroupId(groupId);
    setRecieverImage(image);
    loadGroupMessage(groupId);
  };

  const backLeftSide = () => {
    const chatRightDiv = document.getElementById("chatRightDiv");
    const chatLeftDiv = document.getElementById("chatLeftDiv");
    chatRightDiv?.classList.remove("chatRightDivBlock");
    chatLeftDiv?.classList.remove("chatLeftDivNone");
    chatRightDiv?.classList.add("chatRightDivNone");
    chatLeftDiv?.classList.add("chatLeftDivBlock");
  };

  const scrollChatToBottom = () => {
    const chatContainer = chatContainerRef.current as HTMLDivElement | null;
    if (chatContainer) {
      chatContainer.scrollTop = chatContainer.scrollHeight;
    }
  };

  const scrollGroupToTop = () => {
    const groupContainer = groupContainerRef.current as HTMLDivElement | null;
    if (groupContainer) {
      groupContainer.scrollTop = 0;
    }
  };

  const scrollGroupToBottom = () => {
    const groupContainer = groupContainerRef.current as HTMLDivElement | null;
    if (groupContainer) {
      groupContainer.scrollTop = groupContainer.scrollHeight;
    }
  };

  useEffect(() => {
    scrollChatToBottom();
  }, [messages]);

  const onKeyDownMessage = (e: any) => {
    if (e.key === "Enter") {
      sendMessage();
    }
  };
  const sendOffer = () => {
    const msg = {
      groupId: currentGroupId,
      senderId: loggedInUserId,
      message: "Offer Sent",
      eventId: selectedEvent,
      offer: {
        hours: OfferHour,
        price: OfferPrice,
      },
      type: "OFFER",
      createdAt: new Date(), 
    };
    // setMessages([...messages, msg]);
    setOfferHour("");
    setOfferPrice("");
    socket.emit("send offer", msg);
    loadGroupMessage(currentGroupId);
    scrollChatToBottom();
    scrollGroupToTop();
  };

  const sendMessage = () => {
    if (messageText) {
      const msg = {
        groupId: currentGroupId,
        senderId: loggedInUserId,
        message: messageText,
        type: "TEXT",
        createdAt: new Date(),
      };
      // setMessages([...messages, msg]);
      setMessageText("");
      socket.emit("chat message", msg);
      loadGroupMessage(currentGroupId);
      scrollChatToBottom();
      scrollGroupToTop();
      var input = document.getElementById("InputMsg") as HTMLInputElement;
      if (input) {
        input.value = "";
      }
    }
  };

  const user = async () => {
    try {
      const { success, data } = await CALL_GET_API("user/user-by-id");
      if (success) {
        const userDetails: any = data.data;
        setProvider(userDetails.provider);
        setupSocket(userDetails._id);
      }
    } catch (err) {}
  };

  const showHidePopUp = (e: any) => {
    setEventDetailId(eventDetailId === e._id ? null : e._id);
    setEventInfo(e.eventId);
  };

  const getDate = (dateString: any) => {
    const date = new Date(dateString);
    const day = date.getDate().toString().padStart(2, "0");
    const month = (date.getMonth() + 1).toString().padStart(2, "0");
    const year = date.getFullYear();

    return `${day}/${month}/${year}`;
  };

  const getTime = (time: any) => {
    const date = new Date(time);
    const hours = date.getHours();
    const ampm = hours >= 12 ? "PM" : "AM";
    const formattedHours = hours % 12 || 12;
    return `${formattedHours} ${ampm}`;
  };

  
  useEffect(() => {
    user();
    getEvents();
  }, []);
  
  const getEventInfo = (item: any) => {
    if (item.eventId) {
      return (
        <>
          <small
            style={{
              color: "rgb(19, 180, 151)",
              textDecoration: "underline",
              fontSize: "8px",
              cursor: "pointer",
            }}
            onClick={() => showHidePopUp(item)}
          >
            View Details
          </small>
          <div
            className="absolute eventDetailsPopUp"
            style={{ display: eventDetailId === item._id ? "block" : "none" }}
          >
            <div className="text-black underline text-[14px]">Event Details</div>
            <div className="flex py-3">
              <div className="eventDetailsPopUp-head">Date/Time</div>
              <div className="eventDetailsPopUp-para flex">
                <div>{getDate(eventInfo.startTime)}</div>
                <div className="ml-1">{getTime(eventInfo.startTime)}</div>
              </div>
            </div>
            <div className="flex py-3">
              <div className="eventDetailsPopUp-head">Event Name</div>
              <div className="eventDetailsPopUp-para">
                {eventInfo.eventTitle}
              </div>
            </div>
            <div className="flex py-3">
              <div className="eventDetailsPopUp-head">Address</div>
              <div className="eventDetailsPopUp-para">{eventInfo.Address}</div>
            </div>
            <div className="flex py-3">
              <div className="eventDetailsPopUp-head">City</div>
              <div className="eventDetailsPopUp-para">{eventInfo.city}</div>
            </div>
          </div>
        </>
      );
    } else return "";
  };

  const getMessage = (item: any) => {
    // console.log(item);
    if (item.isStatus) {
      return (
        <div className="flex flex-row items-center w-6/12 gap-2 relative">
          <img
            src={`${router.basePath}/assets/images/person1.png`}
            alt=""
            className="w-8 h-8"
            style={{ borderRadius: "50%" }}
          />
          <div
            className="flex flex-row text-white justify-start w-4/5"
            style={{
              backgroundColor: "#777777",
              fontSize: "12px",
              padding: "8px 10px",
              borderRadius: "8px",
              width: "fit-content",
            }}
          >
            <div>{item.message}</div>
          </div>
          <div className="absolute bottom-1 right-1 text-[.6875rem] mt-1 w-full flex justify-end">
            {item.createdAt && <span> {new Date(item.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' ,hour12: true})}</span>}
          </div>
        </div>
      );
    } else if (item.type == "OFFER") {
      const isMyMessage = loggedInUserId == (item.senderId || {})._id;

      if (isMyMessage) {
        return (
          <div className="flex justify-center">
            <div className="flex flex-col items-center justify-end pr-10 chatsRightSide">
              <div className="flex flex-row items-center offer-popup">
                <img
                  src={
                    recieverImage
                      ? recieverImage
                      : `${router.basePath}/assets/images/ic_user.png`
                  }
                  alt=""
                  className="w-8 h-8 rounded-full mr-4"
                />
                <div className="flex flex-row pl-4 justify-evenly border-l-2 border-white items-center gap-3">
                  <div className="flex flex-col ">
                    <div className="text-white  text-xs">
                      offer Sent for {item?.eventId?.eventTitle}
                    </div>
                    <div className="flex flex-row  text-white  text-xs">
                      ${item.offer.price} | {item.offer.hours} Hour
                    </div>
                  </div>
                  {!item.action ? (
                    <></>
                  ) : (
                    <>
                      <div
                        className="text-white text-xs offerResponse"
                        style={{
                          background:
                            item.action === "Accept" ? "green" : "red",
                        }}
                      >
                        {item.action === "Accept" ? (
                          <div>Accepted</div>
                        ) : (
                          <div>Declined</div>
                        )}
                      </div>
                      <div className="relative">{getEventInfo(item)}</div>
                    </>
                  )}
                </div>
              </div>
              <div className="w-full flex  h-1/4 justify-center self-center"></div>
              <div className="bottom-1 right-1 text-[.6875rem] mt-1 w-full flex justify-end 1">
                {item.createdAt && <span> {new Date(item.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' ,hour12: true})}</span>}
              </div>
            </div>
          </div>
        );
      } else {
        return (
          <div className="flex justify-center">
            <div className="flex flex-col items-center justify-end pr-10">
              <div className="flex flex-row items-center offer-popup">
                <img
                  src={
                    recieverImage
                      ? recieverImage
                      : `${router.basePath}/assets/images/ic_user.png`
                  }
                  alt=""
                  className="w-8 h-8 rounded-full mr-2"
                />
                <div className="flex flex-row pl-4 justify-evenly border-l-2 border-white items-center gap-3">
                  <div className="flex flex-col ">
                    <div className="text-white  text-xs">offer Recieved</div>
                    <div className="flex flex-row  text-white  text-xs">
                      $ {item.offer.price} | {item.offer.hours} Hour
                    </div>
                  </div>

                  {!item.action ? (
                    <div className="flex flex-row gap-3">
                      <div>
                        <button
                          className="text-white text-xs rounded-full bg-emerald-700 px-2 py-1"
                          onClick={(e: any) =>
                            OffersButton("accept", item._id, e)
                          }
                        >
                          Accept
                        </button>
                      </div>
                      <div className="">
                        <button
                          className="text-white text-xs rounded-full bg-indigo-600 px-2 py-1"
                          onClick={(e: any) =>
                            OffersButton("decline", item._id, e)
                          }
                        >
                          Decline
                        </button>
                      </div>
                    </div>
                  ) : (
                    <>
                      <div
                        className="text-white text-xs offerResponse"
                        style={{
                          background:
                            item.action === "Accept" ? "green" : "red",
                        }}
                      >
                        {item.action === "Accept" ? (
                          <div>Accepted</div>
                        ) : (
                          <div>Declined</div>
                        )}
                      </div>
                      <div className="relative">{getEventInfo(item)}</div>
                    </>
                  )}
                </div>
              </div>
              <div className="w-full flex  h-1/4 justify-center self-center"></div>
              <div className="bottom-1 right-1 text-[.6875rem] mt-1 w-full flex justify-end 2">
                {item.createdAt && (
                  <span>
                    {" "}
                    {new Date(item.createdAt).toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
                      hour12: true,
                    })}
                  </span>
                )}
              </div>
            </div>
          </div>
        );
      }
    } else {
      const isMyMessage = loggedInUserId == (item.senderId || {})._id;
      if (isMyMessage) {
        return (
          <div className="flex flex-col items-end self-end w-3/12 ChatText">
            <div
              className="text-white"
              style={{
                backgroundColor: "#13B497",
                fontSize: "12px",
                padding: "8px 10px",
                borderRadius: "8px",
                width: "fit-content",
              }}
            >
              {item.message}
            </div>
            <div className="bottom-1 right-1 text-[.6875rem] mt-1 w-full flex justify-end 3">
              {item.createdAt && <span> {new Date(item.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' ,hour12: true})}</span>}
            </div>
          </div>
        );
      } else {
        return (
          <div className="flex flex-col items-start self-start w-3/12 ChatText">
            <div
              className="text-white"
              style={{
                backgroundColor: "#222297",
                fontSize: "12px",
                padding: "8px 10px",
                borderRadius: "8px",
                width: "fit-content",
              }}
            >
              {item.message}
            </div>
            <div className="bottom-1 right-1 text-[.6875rem] mt-1 w-full flex justify-start">
              {item.createdAt && <span> {new Date(item.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' ,hour12: true})}</span>}
            </div>
          </div>
        );
      }
    }
  };

  return (
    <Layout>
      <div className="w-full">
        <div className="w-full flex boxShadow">
          {/* <div
            className=""
            style={{ display: style ? "none" : "block", width: "100%" }}
          > */}
          <div className="chatLeft " id="chatLeftDiv">
            <div className="chat-head-div-parent">
              <div className="chat-head-div">
                <div className="flex">
                  <img
                    src={`${router.basePath}/assets/images/imgAdd.png`}
                    alt=""
                    className="w-10 h-10 mr-3 rounded-full"
                  />
                  <div>
                    <div className="text-[14px]">Add Contacts</div>
                    <div className="text-xs">
                      You can contact other event staff
                    </div>
                  </div>
                </div>
                <div>
                  <img
                    src={`${router.basePath}/assets/images/arrow.png`}
                    alt=""
                    onClick={() => router.push("/hire-event-staff")}
                    className="rounded-full"
                  />
                </div>
              </div>
              <div
                className="flex items-center my-3 bg-white mx-2 px-4"
                style={{ borderRadius: "50px" }}
              >
                <FiSearch />

                <input
                  type="text"
                  className=" "
                  style={{
                    marginTop: "0",
                    background: "#fff",
                    padding: "6px 20px",
                  }}
                  onKeyUp={handleInputChange}
                />
              </div>
              <div
                className="flex flex-col justify-items-center items-center w-full leftResponsiveHeight"
                style={{ maxHeight: "335px", overflowX: "auto" }}
                ref={groupContainerRef}
              >
                {groups.length === 0 ? (
                  <>
                    <div className="text-sm text-white">No Contacts</div>
                  </>
                ) : (
                  <>
                    {groups.map((group) => (
                      <div
                        className="person-div"
                        id={group._id}
                        style={{
                          background: isCurrentGroup(group._id)
                            ? "rgb(19 180 153 / 50%)"
                            : "",
                        }}
                        onClick={() => reloadGroup(group._id, group.image)}
                      >
                        <div className="flex">
                          <img
                            className="w-10 h-10 rounded-full mr-4"
                            src={
                              group.image
                                ? group.image
                                : `${router.basePath}/assets/images/ic_user.png`
                            }
                          />

                          <div>
                            <div
                              style={{
                                display: "flex",
                                justifyContent: "space-between",
                              }}
                            >
                              <div
                                style={{
                                  fontSize: "14px",
                                  marginBottom: "4px",
                                }}
                              >
                                {" "}
                                @{group.name}{" "}
                              </div>
                            </div>
                            <div
                              className="ellipsis"
                              style={{ fontSize: "14px" }}
                            >
                              {getGroupLastMessageText(group)}
                            </div>
                          </div>
                        </div>

                        <div style={{ fontSize: "10px" }} className="mt-1">
                          {group.lastMessage != null && (
                            <span>
                              {new Date(
                                group.lastMessage.createdAt
                              ).toLocaleTimeString([], {
                                hour: "2-digit",
                                minute: "2-digit",
                                hour12: true,
                              })}
                            </span>
                          )}
                        </div>
                      </div>
                    ))}
                  </>
                )}
              </div>
            </div>
          </div>
          {/* </div> */}

          {/* <div
            className="chatRightParent"
            style={{ display: style ? "block" : "none", width: "100%" }}
          > */}
          {/* <div className="text-white text-lg font-bold" onClick={(e)=>setStyle(false)}>&#129042;</div> */}

          <div className="chatRight" id="chatRightDiv">
            {startChatImg ? (
              <div className="flex justify-center items-center h-full">
                <svg
                  viewBox="0 0 303 172"
                  width="360"
                  preserveAspectRatio="xMidYMid meet"
                  fill="none"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M229.565 160.229C262.212 149.245 286.931 118.241 283.39 73.4194C278.009 5.31929 212.365 -11.5738 171.472 8.48673C115.998 35.6999 108.972 40.1612 69.2388 40.1612C39.645 40.1612 9.51318 54.4147 5.7467 92.952C3.0166 120.885 13.9985 145.267 54.6373 157.716C128.599 180.373 198.017 170.844 229.565 160.229Z"
                    fill="#DAF7F3"
                  ></path>
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M131.589 68.9422C131.593 68.9422 131.596 68.9422 131.599 68.9422C137.86 68.9422 142.935 63.6787 142.935 57.1859C142.935 50.6931 137.86 45.4297 131.599 45.4297C126.518 45.4297 122.218 48.8958 120.777 53.6723C120.022 53.4096 119.213 53.2672 118.373 53.2672C114.199 53.2672 110.815 56.7762 110.815 61.1047C110.815 65.4332 114.199 68.9422 118.373 68.9422C118.377 68.9422 118.381 68.9422 118.386 68.9422H131.589Z"
                    fill="white"
                  ></path>
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M105.682 128.716C109.186 128.716 112.026 125.908 112.026 122.446C112.026 118.983 109.186 116.176 105.682 116.176C104.526 116.176 103.442 116.481 102.509 117.015L102.509 116.959C102.509 110.467 97.1831 105.203 90.6129 105.203C85.3224 105.203 80.8385 108.616 79.2925 113.335C78.6052 113.143 77.88 113.041 77.1304 113.041C72.7503 113.041 69.1995 116.55 69.1995 120.878C69.1995 125.207 72.7503 128.716 77.1304 128.716C77.1341 128.716 77.1379 128.716 77.1416 128.716H105.682L105.682 128.716Z"
                    fill="white"
                  ></path>
                  <rect
                    x="0.445307"
                    y="0.549558"
                    width="50.5797"
                    height="100.068"
                    rx="7.5"
                    transform="matrix(0.994522 0.104528 -0.103907 0.994587 10.5547 41.6171)"
                    fill="#42CBA5"
                    stroke="#316474"
                  ></rect>
                  <rect
                    x="0.445307"
                    y="0.549558"
                    width="50.4027"
                    height="99.7216"
                    rx="7.5"
                    transform="matrix(0.994522 0.104528 -0.103907 0.994587 10.9258 37.9564)"
                    fill="white"
                    stroke="#316474"
                  ></rect>
                  <path
                    d="M57.1609 51.7354L48.5917 133.759C48.2761 136.78 45.5713 138.972 42.5503 138.654L9.58089 135.189C6.55997 134.871 4.36688 132.165 4.68251 129.144L13.2517 47.1204C13.5674 44.0992 16.2722 41.9075 19.2931 42.2251L24.5519 42.7778L47.0037 45.1376L52.2625 45.6903C55.2835 46.0078 57.4765 48.7143 57.1609 51.7354Z"
                    fill="#EEFEFA"
                    stroke="#316474"
                  ></path>
                  <path
                    d="M26.2009 102.937C27.0633 103.019 27.9323 103.119 28.8023 103.21C29.0402 101.032 29.2706 98.8437 29.4916 96.6638L26.8817 96.39C26.6438 98.5681 26.4049 100.755 26.2009 102.937ZM23.4704 93.3294L25.7392 91.4955L27.5774 93.7603L28.7118 92.8434L26.8736 90.5775L29.1434 88.7438L28.2248 87.6114L25.955 89.4452L24.1179 87.1806L22.9824 88.0974L24.8207 90.3621L22.5508 92.197L23.4704 93.3294ZM22.6545 98.6148C22.5261 99.9153 22.3893 101.215 22.244 102.514C23.1206 102.623 23.9924 102.697 24.8699 102.798C25.0164 101.488 25.1451 100.184 25.2831 98.8734C24.4047 98.7813 23.5298 98.6551 22.6545 98.6148ZM39.502 89.7779C38.9965 94.579 38.4833 99.3707 37.9862 104.174C38.8656 104.257 39.7337 104.366 40.614 104.441C41.1101 99.6473 41.6138 94.8633 42.1271 90.0705C41.2625 89.9282 40.3796 89.8786 39.502 89.7779ZM35.2378 92.4459C34.8492 96.2179 34.4351 99.9873 34.0551 103.76C34.925 103.851 35.7959 103.934 36.6564 104.033C37.1028 100.121 37.482 96.1922 37.9113 92.2783C37.0562 92.1284 36.18 92.0966 35.3221 91.9722C35.2812 92.1276 35.253 92.286 35.2378 92.4459ZM31.1061 94.1821C31.0635 94.341 31.0456 94.511 31.0286 94.6726C30.7324 97.5678 30.4115 100.452 30.1238 103.348L32.7336 103.622C32.8582 102.602 32.9479 101.587 33.0639 100.567C33.2611 98.5305 33.5188 96.4921 33.6905 94.4522C32.8281 94.3712 31.9666 94.2811 31.1061 94.1821Z"
                    fill="#316474"
                  ></path>
                  <path
                    d="M17.892 48.4889C17.7988 49.3842 18.4576 50.1945 19.3597 50.2923C20.2665 50.3906 21.0855 49.7332 21.1792 48.8333C21.2724 47.938 20.6136 47.1277 19.7115 47.0299C18.8047 46.9316 17.9857 47.5889 17.892 48.4889Z"
                    fill="white"
                    stroke="#316474"
                  ></path>
                  <path
                    d="M231.807 136.678L197.944 139.04C197.65 139.06 197.404 139.02 197.249 138.96C197.208 138.945 197.179 138.93 197.16 138.918L196.456 128.876C196.474 128.862 196.5 128.843 196.538 128.822C196.683 128.741 196.921 128.668 197.215 128.647L231.078 126.285C231.372 126.265 231.618 126.305 231.773 126.365C231.814 126.381 231.842 126.395 231.861 126.407L232.566 136.449C232.548 136.463 232.522 136.482 232.484 136.503C232.339 136.584 232.101 136.658 231.807 136.678Z"
                    fill="white"
                    stroke="#316474"
                  ></path>
                  <path
                    d="M283.734 125.679L144.864 135.363C141.994 135.563 139.493 133.4 139.293 130.54L133.059 41.6349C132.858 38.7751 135.031 36.2858 137.903 36.0856L276.773 26.4008C279.647 26.2005 282.144 28.364 282.345 31.2238L288.578 120.129C288.779 122.989 286.607 125.478 283.734 125.679Z"
                    fill="white"
                  ></path>
                  <path
                    d="M144.864 135.363C141.994 135.563 139.493 133.4 139.293 130.54L133.059 41.6349C132.858 38.7751 135.031 36.2858 137.903 36.0856L276.773 26.4008C279.647 26.2004 282.144 28.364 282.345 31.2238L288.578 120.129C288.779 122.989 286.607 125.478 283.734 125.679"
                    stroke="#316474"
                  ></path>
                  <path
                    d="M278.565 121.405L148.68 130.463C146.256 130.632 144.174 128.861 144.012 126.55L138.343 45.695C138.181 43.3846 139.994 41.3414 142.419 41.1723L272.304 32.1142C274.731 31.945 276.81 33.7166 276.972 36.0271L282.641 116.882C282.803 119.193 280.992 121.236 278.565 121.405Z"
                    fill="#EEFEFA"
                    stroke="#316474"
                  ></path>
                  <path
                    d="M230.198 129.97L298.691 125.193L299.111 131.189C299.166 131.97 299.013 132.667 298.748 133.161C298.478 133.661 298.137 133.887 297.825 133.909L132.794 145.418C132.482 145.44 132.113 145.263 131.777 144.805C131.445 144.353 131.196 143.684 131.141 142.903L130.721 136.907L199.215 132.131C199.476 132.921 199.867 133.614 200.357 134.129C200.929 134.729 201.665 135.115 202.482 135.058L227.371 133.322C228.188 133.265 228.862 132.782 229.345 132.108C229.758 131.531 230.05 130.79 230.198 129.97Z"
                    fill="#42CBA5"
                    stroke="#316474"
                  ></path>
                  <path
                    d="M230.367 129.051L300.275 124.175L300.533 127.851C300.591 128.681 299.964 129.403 299.13 129.461L130.858 141.196C130.025 141.254 129.303 140.627 129.245 139.797L128.987 136.121L198.896 131.245C199.485 132.391 200.709 133.147 202.084 133.051L227.462 131.281C228.836 131.185 229.943 130.268 230.367 129.051Z"
                    fill="white"
                    stroke="#316474"
                  ></path>
                  <ellipse
                    rx="15.9969"
                    ry="15.9971"
                    transform="matrix(0.997577 -0.0695704 0.0699429 0.997551 210.659 83.553)"
                    fill="#42CBA5"
                    stroke="#316474"
                  ></ellipse>
                  <path
                    d="M208.184 87.1094L204.777 84.3593C204.777 84.359 204.776 84.3587 204.776 84.3583C203.957 83.6906 202.744 83.8012 202.061 84.6073C201.374 85.4191 201.486 86.6265 202.31 87.2997L202.312 87.3011L207.389 91.4116C207.389 91.4119 207.389 91.4121 207.389 91.4124C208.278 92.1372 209.611 91.9373 210.242 90.9795L218.283 78.77C218.868 77.8813 218.608 76.6968 217.71 76.127C216.817 75.5606 215.624 75.8109 215.043 76.6939L208.184 87.1094Z"
                    fill="white"
                    stroke="#316474"
                  ></path>
                </svg>
              </div>
            ) : (
              <>
                {loader ? (
                  <div className="flex justify-center items-center h-full">
                    <AppLoader />
                  </div>
                ) : (
                  <>
                    <div className="leftArrow" onClick={backLeftSide}>
                      <FiArrowLeft />
                    </div>
                    <div className="h-4/5 flex flex-col justify-end chatsCol">
                      <div
                        className="flex flex-col w-full gap-4 forScrolling mb-5"
                        style={{ padding: "220px 24px 0 24px" }}
                        ref={chatContainerRef}
                      >
                        {messages.map((item) => getMessage(item))}
                      </div>
                    </div>
                    <div className="w-full flex flex-col typeingDiv">
                      <div className="flex flex-col items-center justify-end gap-2">
                        {!provider ? (
                          <div
                            className="flex flex-row w-full gap-3 items-end sendOfferDiv"
                            style={{ padding: "0 70px" }}
                          >
                            <div
                              className="flex flex-row w-2/4 justify-evenly rounded-2xl"
                              style={{ backgroundColor: "#13B497" }}
                            >
                              <div className="flex items-end">
                                <span className="text-black mb-1 text-xs">
                                  $
                                </span>
                                <input
                                  className="offerInput"
                                  type="number"
                                  min={0}
                                  value={OfferPrice}
                                  onChange={(e: any) =>
                                    sendHourMoney(e, "forMoney")
                                  }
                                />
                                <div className="menu-img">
                                  <img
                                    src={`${router.basePath}/assets/images/ic_chevron_down.png`}
                                  />
                                </div>
                              </div>

                              <div className="flex items-end">
                                <input
                                  className="offerInput"
                                  type="number"
                                  value={OfferHour}
                                  onChange={(e: any) =>
                                    sendHourMoney(e, "forHour")
                                  }
                                />
                                <span className="text-black mb-1 text-xs">
                                  Hour
                                </span>

                                <div className="menu-img">
                                  <img
                                    src={`${router.basePath}/assets/images/ic_chevron_down.png`}
                                  />
                                </div>
                              </div>
                            </div>
                            <div
                              className="flex flex-row  justify-center  w-1/4  rounded-2xl h-full"
                              style={{ backgroundColor: "#13B497" }}
                            >
                              <div>
                                <select
                                  onChange={(e: any) =>
                                    setSelectedEvent(e.target.value)
                                  }
                                  placeholder="Service Category"
                                  className="w-full h-full rounded-2xl text-xs"
                                  style={{
                                    color: "black",
                                    backgroundColor: "#13B497",
                                  }}
                                >
                                  <option>Select Event</option>
                                  {events.map((e: any) => (
                                    <option value={e._id}>
                                      {e.eventTitle}
                                    </option>
                                  ))}
                                </select>
                              </div>
                            </div>
                            <div className="flex flex-row h-6   text-white  underline w-1/4">
                              <button
                                disabled={
                                  selectedEvent === "" ||
                                  selectedEvent === "Select Event" ||
                                  OfferPrice === "" ||
                                  OfferHour === ""
                                }
                                className="flex-row cursor-pointer sendOfferText"
                                onClick={sendOffer}
                              >
                                Send Offer
                              </button>
                            </div>
                          </div>
                        ) : (
                          <></>
                        )}
                        <div className="chat-input">
                          <div className="flex flex-row w-full items-center h-12 gap-4">
                            <input
                              className="h-full w-fit"
                              type="text"
                              name="InputMsg"
                              id="InputMsg"
                              // value={messageText}
                              style={{
                                marginTop: "0",
                                color: "#fff",
                                background: "#272E35",
                              }}
                              onChange={sendReply}
                              // onChange={(e:any)=>setMessageText(e.target.value)}
                              onKeyDown={onKeyDownMessage}
                              placeholder="type message"
                            />

                            <FiSend
                              size={28}
                              style={{ color: "#fff", cursor: "pointer" }}
                              onClick={sendMessage}
                            />
                          </div>

                          {/* {loggedInUserId === currentGroupId ? (
                            <>
                            id Matched
                            </>
                          ):(
                            <>
                            id not matched</>
                          )} */}

                          {/* {loggedInUserId != currentGroupId ? (
                            <>
                              {isTyping ? (
                                <div className="text-xs text-white">Typing...</div>
                              ) : (
                                <></>
                              )}
                            </>
                          ) : (
                            <></>
                          )} */}
                        </div>
                      </div>
                    </div>
                  </>
                )}
              </>
            )}

            {/* <div className="h-full flex flex-col justify-end ">
              <div
                className="flex flex-col w-full gap-4 forScrolling "
                style={{ padding: "24px 24px 0 24px" }}
              >
                {messages.map((item) => getMessage(item))}
              </div>
            </div> */}
          </div>
          {/* </div> */}
        </div>
      </div>
    </Layout>
  );
};

export default chatBox;
